...
var promises = {
	...
	var next = this.args[1];
        ...
        if (res.length < 1) {
            this.error('no results for "'+selector+'" in '+context.doc().request.url);
            self.next.start(null);
	    return;
        }
        
        this.log('found '+res.length+' results for "'+selector+'"'+' in '+context.doc().request.url)
        var i = 0;
    ...
    set: function(context, data, dataPath) {
	var self = this;
	var obj = this.args;
	var path = data;
	...
	if (!keys.some(function(key, i) {
		...
		path[key] = [];
		dataPath.push(key);
		self.cb(context, data, dataPath);
		return true;
	    }else if (val !== null) {
		if (!isArray)
		path[key] = {};
		if (val.isPromise === true) {
	...
	if (this.args[3] !== undefined) {
	    var cb = function(c) {
		self.args[3].call(self, c, data, function(nc, nd) {
		    self.next.start(nc||c, nd||data)
		});
	    }
	}else{
	    var cb = function(c) { self.next.start(c, data) };
	}
	...
	var userInput	= form.get('input[type="password"]');
	var passInput	= form.get('input:before(input[type="password"]):last');
