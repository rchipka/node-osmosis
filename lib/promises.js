...
var Promise = {
    ...
    click: function(context, data, next, done) {
        ...
        if (nodes.length === 0) {
            this.debug('no results for "'+selector+'" in '+location);
            return done();
        }
        var self = this;
    ...
    find: function(context, data, next, done) {
        ...
        if (res.length < 1) {
            this.error('no results for "' + selector + '"' + location);
            done(context, data);
            return;
        }

        this.log('found ' + res.length + ' results for "' + selector + '"' + location)
        var i = 0;
    ...
    follow: function(context, data, next, done) {
        ...
        if (doc.request !== undefined)
            location = doc.request.url;
        if ((typeof context.name).charAt(0) === 'f')
            location += ' (' + context.path() + ')';
        else
            location += ' (' + doc.root().path() + ')';
        if (location.length !== 0)
            location = ' in ' + location

        if (res === undefined || res.length === 0) {
            this.debug('no results for "' + selector + '"' + location);
            done(context, data);
            return;
        } else {
            var opts = {
            ...
            res.forEach(function(el, i) {
                ...
                if (val !== null) {
                    ...
                    count++;
                    self.log("url: " + url)
                    self.request('get', url, undefined, function(err, document) {
                        next(document, data);
                        if (--count === 0)
                            done();
                    }, opts)
...
Promise.post.preloader = function(args) {
    if (typeof args[0] === 'string' && isFormattedString(args[0])) {
        args[0] = parseFormattedString(args[0]);
    }
    for (var i = 1; i < 3; i++) {
...
function isFormattedString(str) {
    return str.search(/[\%\$]\{[^\}]+\}/) !== -1;
}
...
function parseFormattedString(str) {
    ...
    str = str.replace(/(.*?)([\%\$]\{([^\}]+)\})/g, function(m, a, b, c) {
        if (a.length !== 0)
            f = f+'"'+a+'"+';
        f += (b.charAt(0) === '$')?'context.get("'+c+'").content()+':'data[\"'+c+'\"]+';
        return '';
    })
    f += '"'+str+'"';
