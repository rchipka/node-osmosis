...
var Promise = {
    ...
    find: function (context, data, next, done) {
        ...
        if (this.name[0] === 's' || this.isNestedPromise === true)
    ...
    set: function (context, data, next) {
        ...
        var loopObject = function () {
            ...
            if (type === 'object' && val !== null) {
                ...
                if (val === null) {
                    data.set(key, getContent(context));
                } else if (type === 'function') {
                    data.set(key, val(context));
                } else if (Array.isArray(args)) {
                    var nodes  = context.find(val),
                        length = nodes.length;

                    if (length !== 0) {
                        for (var node = 0; node < length; node++) {
                            data.push(getContent(nodes[node]));
                        }
                    }

                    nodes = null;
                } else {
                    var node = context.get(val);

                    if (node !== null)
                        data.set(key, getContent(node));
                    node = null;
                }
    ...
    then: function (context, data, next, done) {
        ...
        if (this.useDocument !== undefined) {
            ...
            cb = function () {
                ...
                window.addEventListener('done', function (ev) {
                    ...
                    if (length > 2) {
                        cbOrig.call(self, c, data.obj, next, done);
                    } else {
                        cbOrig.call(self, c, data.obj);
                        next(context, data);
                        done();
                    }
                })
        ...
        if (length === 4 || this.useWindow !== undefined) {
...
var regexp_token_string = /[\%\$]\{[^\}]+\}/;
var regexp_token_parse = /(.*?)([\%\$]\{([^\}]+)\})/g;

function isTokenizedString(str) {
    return str.search(regexp_token_string) !== -1;
}

function parseTokenizedString(str) {
    var f = 'return ';

    str = str.replace(regexp_token_parse, function (m, a, b, c) {
        if (a.length !== 0)
            f = f + '"' + a + '"+';
...
function formQueryParams(form, submit) {
    var doc = form.doc();

    if (form.getAttribute('type') === 'submit' && (form.nodeName === 'input' || form.nodeName === 'button')) {
        var submit = form;
        var form_id = submit.getAttribute('form');

        if (form_id !== null)
            form = submit.doc().getElementById(form_id);
        else
            form = form.get('ancestor::form');

    } else {
        var submit = form.get('[@type="submit" and not(@disabled) and (not(@form) or @form="' + form.getAttribute('id') + '")][1]');
    }
    ...
    var inputs = form.find('[@name and not(@disabled) and not(@type="submit")]');
    ...
    if (submit !== null) {
        if (submit.hasAttribute('name'))
            params[submit.getAttribute('name')] = submit.getAttribute('value') || 'Submit Query';

        if (submit.hasAttribute('formaction'))
            obj.action = URL.resolve(obj.action, submit.getAttribute('formaction'));

        if (submit.hasAttribute('formmethod'))
            obj.method = submit.getAttribute('formmethod');

        if (submit.hasAttribute('formenctype'))
            obj.enctype = submit.getAttribute('formenctype');
    }
