...
function Request(method, url, params, opts, tries, callback) {
    ...
    return needle.request(method,
        ...
        if (opts.ignore_http_errors !== true &&
            res                     !== undefined &&
            isErrorStatusCode(res.statusCode, opts.error_status_codes)) {
            // HTTP error
        ...
        location.user_agent = typeof opts.user_agent === "function" ? opts.user_agent() : opts.user_agent;
...
function isErrorStatusCode(response_error_code, error_status_codes) {

    error_status_codes = error_status_codes.split(',');
    for (var index = 0, len = error_status_codes.length; index < len; ++index) {
        var error_code = error_status_codes[index];
        if (error_code.indexOf('-') > -1) {
            var from_to = error_code.split('-').map(function (ec) { return parseInt(ec)});
            if (response_error_code >= from_to[0] && response_error_code <= from_to[1]) {
                return true;
            }
        }
        if (response_error_code === parseInt(error_code)) {
            return true;
        }
    }
    return false;
