...
var css2xpath = (function(){
    var re      = [
            ...
            /\s+\/\//g, '//',
            // :not
            /:not\(([^\)]*)[\)]+/g, function(s, a){ if (a.indexOf('(') !== -1) a += ')'; return String().concat("[not(", (css2xpath(a).replace(/^[^\[]*\[([^\]]*)\].*$/g, "$1")), ")]"); },
            // :first-child
            ...
            /*return "[contains(css:lower-case(string(.)),'" + a.toLowerCase() + "')]"; // it does not work in firefox 3*/
                return "[contains(string(.),'" + a.replace("'", '\\\'') + "')]";
            },
            // |= attrib
            ...
            // :starts-with
            /:starts-with\(['"]?([^\)]*?)['"]?\)/g, function(s, a){
                return "[starts-with(string(.),'" + a.replace("'", '\\\'') + "')]";
            },
            // :ends-with
            /:ends-with\(['"]?([^\)]*?)['"]?\)/g, function(s, a){
                return "[ends-with(string(.),'" + a.replace("'", '\\\'') + "')]";
            },
            // :first(n), limit(n)
