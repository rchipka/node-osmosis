...
Data.prototype = {
	...
	next: function(obj, index, done) {
		return (new Data(obj, this, index, done)).ref();
	},
}
...
Promise.prototype.setNestedData = function(context, data, next) {
	if (this.hasSet === false && typeof data.obj === 'object' && Object.keys(data.obj).length === 0) { // If a value wasn't set, then set to the current context value
		if (context.text !== undefined)
			data.obj = context.text();
		else if (context.value !== undefined)
			data.obj = context.value();
	}
	...
	this.instance.request(method, url, params, function(err, res, document, last_try) {
		if (err !== null) {
			...
			if (last_try === true) { // Call callback after **final** request only
				...
				self.instance.stack.pop();
				// We must call stack.pop() again here because
				// stack.push() is incremented **twice** for each retry.
				// First before the retry request is queued,
				// and then before the retry request is executed.
			}
		}else{
